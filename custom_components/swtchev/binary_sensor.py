...
async def async_setup_entry(
    ...
    async_add_entities([SwtchBinarySensorEntity(coordinator, description) for description in BINARY_SENSORS])
...
class SwtchBinarySensorEntity(SwtchCoordinatorEntity, BinarySensorEntity):
    ...
    def is_on(self):
        ...
        if desc.value_type == "connected":
            cp_status = nested_get(data, ("data", "csInfo", "evses", 0, "connectors", 0, "cpStatus"))
            if cp_status is None:
                return None
            return cp_status not in ["Available", "Unavailable"]

        if desc.value_type == "active":
            current = nested_get(data, ("data", "csInfo", "evses", 0, "connectors", 0, "current"), 0)
            try:
